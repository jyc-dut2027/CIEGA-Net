...
if train_or_eval is not True:
    # 测试的配置
    task = 'denoising'
    dataset_dir = r'E:\Dataset\Forgta\zffevent\original\event7\00009'  # 测试图片包括边缘图的路径
    dataset_gtc_dir = r'E:\Dataset\Forgta\zffevent\original\event7\00009'
    # 相对应的gtc路径(使用了训练集中的gtc)，所有设置的路径都是00006/1这种文件夹的父文件夹才可以
    out_img_dir = r'E:\Dataset\Forgta\zffevent\derain\event7\00009'  # 实验结果存放位置
    pathlistfile = r'E:\Dataset\Forgta\zffevent\original\event7\9.txt' # 测试图片具体路径
    model_path = 'model_path/deraining.pkl'
    ...
    # 训练的配置
    task = 'denoising'
    edited_img_dir = r'E:\Diydataset\Forgta\frametrain'  # 训练输入的图片的文件夹
